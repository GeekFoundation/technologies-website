...
import { html }      from '../config/app.js'; // Configuration object for the HTML task
...
const htmlTask = () => gulp.src(html.src, { base : html.base })
	.pipe(plumber({
	...
	.pipe(data((file) => {
		const filePath = `${html.data + path.basename(file.path)}.json`;
